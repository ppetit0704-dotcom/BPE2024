...
def affiche_selecteur_bpe(df: pd.DataFrame) -> pd.DataFrame | None:
    ...
    if mode == "Commune":
        ...
        col_ter = "EPCI"
        label   = "EPCI (code)"
        # Certains EPCI peuvent manquer — on affiche code + libcom première commune
        choix   = sorted(df[col_ter].dropna().unique())
    st.write("Types uniques :", {type(x) for x in choix})
    print("Types uniques :", {type(x) for x in choix})
